...
const AGENT_RESULT_VERSION = 'AGENT_RESULT_JSON_V1';
...
export function validateCanonicalAgentResult(result, options = {}) {
  ...
  if (options.expectedHandoff !== undefined || result.handoff !== undefined) {
    try { validateAgentHandoff(result, { expectedHandoff: options.expectedHandoff, orderId: expectedOrderId ?? result.orderId }); } catch (error) { failures.push(error.message); }
  }
